<template>
  ...
  <div class="msg" ref="msgref" :style="[style1, style2]">
    ...
    <!-- 多个动态样式 -->
    <div :class="[activeClass, activeClass1]">{{ tags }}</div>
    <!-- 动态判断 -->
    <div :class="{ activeClass: isActive }">{{ tags }}</div>
    <!-- 多个判断 -->
    <div :class="[{ activeClass: isActive }, { activeClass2: !isActive }]">
      {{ tags }}
    </div>
    <router-link :to="{ name: 'about1' }">demo/about</router-link>
    <router-view />
    <Tab></Tab>
    <GrandFather />
  </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    console.log("获取路由名称", route.name);

    console.log("路由参数", route.params, route.query);

    const MATCHED = route.matched;
    const LEN = MATCHED.length;

    console.log("路由记录", MATCHED);

    console.log("路由个数", LEN);

    console.log("父级路由", MATCHED[LEN - 2]);
